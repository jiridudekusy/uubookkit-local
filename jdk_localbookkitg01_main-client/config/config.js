...
//not supported by java server    appAssetsRelativeUrlPath: (isWithoutServer ? "" : "public/" + pkg.version + "/"), // URL path (relative to appBaseUrlPath) where built client-side files are deployed
    ...
    aliases: {
      "calls": ((!isProductionBuild && useMockData) ? path.resolve(srcDir, "../mock/mocks.js") : path.resolve(srcDir, "calls.js")),
      "namespace": buildHelpers.createNamespaceAliasFileForJs(),
      "css-namespace": buildHelpers.createNamespaceAliasFileForCss(),
    }
  };
