...
function getWebpackConfig(options) {
  ...
  var plugins = [
    ...
    new webpack.DefinePlugin({
      VERSION: JSON.stringify(pkg.version), // @deprecated use process.env.VERSION instead
      "process.env": {
  ...
  if (opts.copyFiles) {
    ...
    (!opts.isProductionBuild) && plugins.push(new CopyWebpackPlugin([{ context: "../", from: "mock/**" }]));
    ...
    if (preserveSegmentsMatch && (opts.appBaseUrlPath || opts.appAssetsRelativeUrlPath)) {
      ...
      htmlTemplatePostProcessingFn = (template => {
        ...
        template = template.replace(/(<script[^>]*\bsrc\s*=\s*['"])([^'"]*)(['"][^>]*>\s*<\/script>)/gi, (m, g1, url, g3) => {
          ...
          if (url.match(/^\/|^[a-zA-Z0-9\\-_]*:/)) return m;
          let needsAbsolute = g1.match(/\srel=['"](?:shortcut )?icon['"]/); // IE needs absolute URL for favicon (ignores document.baseURI)
          return (
            needsAbsolute
            ? `<script>document.write(${JSON.stringify(g1)} + appBaseUrlPath + appAssetsRelativeUrlPath + ${JSON.stringify(url + g3)});</script>`
            : `<script>document.write(${JSON.stringify(m)});</script>`
          );
        }).replace(/(<meta\s+name=['"]msapplication-(?:config|TileImage|\w+logo)['"]\s+content=['"])([^'"]*)(['"][^>]*>)/gi, (m, g1, url, g3) => {
          ...
          return url.match(/^\/|^[a-zA-Z0-9\\-_]*:/) ? m : `<script>document.write(${JSON.stringify(g1)} + appBaseUrlPath + appAssetsRelativeUrlPath + ${JSON.stringify(url + g3)});</script>`;
        });
  ...
  var devServerConfig = {
    ...
    setup: function (app) {
      ...
      if (Object.keys(mappingsJson || {}).length > 0) {
        ...
        // create list of UVE mappings with matcher function ("matchFn") and the rule relevancy, such as:
        //   home/xy
        //   home/
        //   home
        //   home/{asdf}/{qwer}
        //   home/{asdf}
        var ucList = Object.keys(ucMap).filter(k => ucMap[k].type === "UVE").map(k => {
          if (k === "defaultUve") return { key: k, matchFn: (/*url*/) => true, relevancy: 0, value: ucMap[k] };
          var relX = 1;
...
function getHtmlFilesFromMappingsJson() {
  ...
  for (var k in mappingsJson) {
    // iterate over UCs in the mappings.json and pick those which are UVE and end with .html (or .htm)
    var ucMap = (mappingsJson[k] || {})["useCaseMap"] || {};
    Object.keys(ucMap)
      .filter(uc => ucMap[uc].type === "UVE" && (ucMap[uc].realization||"").match(/\.html?$/i))
...
function createNamespaceAliasFileForJs() {
  var nsConfig = `const namespace = {
    namespace: "${nsApp}",
    cssPrefix: "${nsCss}",
    tag: function(component) {return namespace.namespace + "." + component;},
    css: function(component) {return namespace.cssPrefix + "-" + component;}
  };
  export default namespace;`;
  return createTemporaryModuleFile(nsConfig, "namespace.js");
}
function createNamespaceAliasFileForCss() {
  var nsLess = `@cssNamespace: ${nsCss};`;
  return createTemporaryModuleFile(nsLess, "css-namespace.less");
}
